...
#### Create the webapp and deploy it in bluemix suing the cf tool
...
2.  Login the directory to use the CF tool
 cf login -a -s dev https:// 
	
3. Deploy  it to bluemix
cf push MCDOapp-<yourname> -m 512M pb build/libs/myapp.war

#### Bind required services in bluemix.
